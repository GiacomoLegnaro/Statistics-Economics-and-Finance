...
#mainDir="/Users/giacomolegnaro/Documenti - Mega/Statistica, Economia e Finanza/Tesi/Codice/004 - Conditionally AR(1)-APARCH(1,1)"
mainDir="C:/Giacomo/004 - Conditionally AR(1)-APARCH(1,1)"
setwd(mainDir)
subDir="Output"
...
for (y in STOCK){
  ...
  backTestVaR.cond <- function(x, p = 0.95) {
    ...
    normal.VaR = as.numeric(VaR.araparch(x, p=p, d="norm"))
    tstudent.VaR = as.numeric(VaR.araparch(x, p=p, d="std"))
    skewt.VaR = as.numeric(VaR.araparch(x, p=p, d="sstd"))
    sged.VaR = as.numeric(VaR.araparch(x, p=p, d="sged"))
  ...
  for (z in 1:3){
    ...
    for(i in colnames(VaR.results)) {
      ...
      chart.TimeSeries(-VaR.results[,i], 
                       #legend.loc="topright",
                       main=paste("Unconditional AR(1)-APARCH(1,1) - ", i,
  ...
  write.csv2(VaR.performance, file="VaR Prediction Performance - GARCH models - AR(1)-APARCH(1,1).csv")
