...
# Funzioni
########################################################################
# VaR - Library PerformanceAnalytics
VaR = function (R = NULL, p = 0.95, ..., method = c("modified", "gaussian", 
                                                    "historical", "tstud", "skewt",
                ...
                invert = TRUE) 
{
  ...
  if (!is.null(R)) {
    ...
    if (is.null(mu)) 
      stop("Nothing to do! You must pass either R or the moments mu, sigma, etc.")
  ...
  switch(portfolio_method, single = {
    if (is.null(weights)) {
      switch(method, modified = {
        rVaR = VaR.CornishFisher(R = R, p = p)
      }, gaussian = {
        rVaR = VaR.Gaussian(R = R, p = p)
      }, historical = {
        rVaR = -1 * t(apply(R, 2, quantile, type=3, probs = 1 - p, na.rm = TRUE))
      }, tstud = {
        rVaR = VaR.tstud(R = R, p = p)
      }, skewt = {
        rVaR = VaR.skewt(R = R, p = p)
      }, kernel = {
      ...
      switch(method, modified = {
        rVaR = mVaR.MM(w = weights, mu = mu, sigma = sigma, 
                       M3 = m3, M4 = m4, p = p)
      }, gaussian = {
        rVaR = GVaR.MM(w = weights, mu = mu, sigma = sigma, 
                       p = p)
      }, historical = {
        rVaR = VaR.historical(R = R, p = p) %*% weights
      })
    }
    columns <- ncol(rVaR)
    for (column in 1:columns) {
      tmp = rVaR[, column]
      if (!is.finite(tmp)) (next)()
      if (eval(tmp < 0)) {
        message(c("VaR calculation produces unreliable result (inverse risk) for column: ", 
                  column, " : ", rVaR[, column]))
        rVaR[, column] <- NA
      } else if (eval(1 < tmp)) {
        message(c("VaR calculation produces unreliable result (risk over 100%) for column: ", 
                  column, " : ", rVaR[, column]))
        rVaR[, column] <- 1
      }
    }
    ...
    switch(method, modified = {
      return(VaR.CornishFisher.portfolio(p, weights, mu, 
                                         sigma, m3, m4))
    }, gaussian = {
      ...
      return(VaR.tstud.portfolio(R, p, weights))
    }, kernel = {
...
###############################################################################
# PortfolioRisk.R
# https://github.com/R-Finance/PerformanceAnalytics/blob/6e32fa7d55b45d32941b5c3fd4579d316b04fe9e/R/PortfolioRisk.R
###############################################################################
...
VaR.kernel.portfolio =  function( R, p, w )
{
  ...
  CVaR = c();
  VaR = -quantile( portfolioreturn, type=3, probs = alpha );
...
ES.historical = function(R,p) {
  ...
  for(column in 1:ncol(R)) {
    ...
    q = quantile(r, type=3, probs=alpha)
    exceedr = r[r<q]
    hES = (-mean(exceedr))
    if(is.nan(hES)){
