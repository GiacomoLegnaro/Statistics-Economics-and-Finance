...
#setwd("/Users/giacomolegnaro/Documenti - Mega/Statistica, Economia e Finanza/Tesi/Codice")
#setwd("/Users/giacomolegnaro/Documenti - Mega/Statistica, Economia e Finanza/Tesi/Codice/002 - Conditionally AR(1)-GARCH(1,1)/Output")
#setwd("/Users/giacomolegnaro/Documenti - Mega/Statistica, Economia e Finanza/Tesi/Codice/002 - Conditionally AR(1)-GARCH(1,1)")

#setwd("/Volumes/Untitled/Tesi/Codice/WorkSpace")
#setwd("I:/Tesi/Codice/WorkSpace")

#mainDir="/Users/giacomolegnaro/Documenti - Mega/Statistica, Economia e Finanza/Tesi/Codice/002 - Conditionally AR(1)-GARCH(1,1)"
mainDir="C:/Giacomo/002 - Conditionally AR(1)-GARCH(1,1)"
setwd(mainDir)
subDir="Output"
...
for (i in STOCK){
  ...
  backTestVaR.cond <- function(x, p = 0.95) {
    ...
    normal.VaR = as.numeric(VaR.argarch(x, p=p, d="norm"))
    tstudent.VaR = as.numeric(VaR.argarch(x, p=p, d="std"))
    skewt.VaR = as.numeric(VaR.argarch(x, p=p, d="sstd"))
    sged.VaR = as.numeric(VaR.argarch(x, p=p, d="sged"))
    #normal.VaR = VaR.argarch(x, p=p, d="norm")
    #tstudent.VaR = VaR.argarch(x, p=p, d="std")
    #skewt.VaR = VaR.argarch(x, p=p, d="sstd")
    #sged.VaR = VaR.argarch(x, p=p, d="sged")
    #normal.VaR.n = as.numeric(normal.VaR)
    #tstudent.VaR.n = as.numeric(tstudent.VaR)
    #skewt.VaR.n = as.numeric(skewt.VaR)
    #sged.VaR.n = as.numeric(sged.VaR)
    #Se si vuol avere il valore del percentile originario come segno invert=FALSE
    #ans = cbind(normal.VaR.n, tstudent.VaR.n, skewt.VaR.n, sged.VaR.n)
    ans = cbind(normal.VaR, tstudent.VaR, skewt.VaR, sged.VaR)
    #head(ans)
    #ans = c(normal.VaR, tstudent.VaR, skewt.VaR)
    #names(ans) = c("Normal", "t-stud", "skew-t")
  ...
  for (z in 1:3){
    ...
    chart.TimeSeries(merge(STOCK.ret, VaR.results), legend.loc="topright",
                     main=paste("VaR livello di significatività ", 1-alpha, sep=""))
    dev.off() #Chiusura file postscript
    ...
    for(i in colnames(VaR.results)) {
      ...
      chart.TimeSeries(-VaR.results[,i], 
                       #legend.loc="topright",
                       main=paste("Unconditional AR(1)-GARCH(1,1) - ", i,
  ...
  write.csv2(VaR.performance, file="VaR Prediction Performance - GARCH models - AR(1)-GARCH(1,1).csv")
